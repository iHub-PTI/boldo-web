    ...
    <>
      <Link href='#' variant='body1'>
        <Button
          aria-controls='fade-menu'
          aria-haspopup='true'
          style={{ color: '#F08F77', textDecorationLine: 'underline', textTransform: 'lowercase' }}
          onClick={handleClick}
        >
          editar
        </Button>
              ...
              >
                <Fade in={openC}>
                  ...
                  >
                    ...
                    <Button id='transition-modal-description'>05:00 pm</Button>
                    <Button id='transition-modal-description'>05:30 pm</Button>
                    <Button style={{ backgroundColor: '#27BEC2', color: 'white' }} id='transition-modal-description'>
                      Confirmar
                    </Button>
                  </div>
                </Fade>
...
export function AppointmentType(props, date) {
  const dateCalendar = date.date
  ...
  if (appointmentStart.split('T')[1].split('.')[0].split(':')[0] < 12) {
    if (appointmentStart.split('T')[0] === outputDate()) {
        ...
        if (appointmentStatus === 'open') {
          return (
            <div style={{ padding: '0.5rem' }}>
              <Card
                variant='outlined'
                style={{ display: 'flex', borderRadius: '16px', justifyContent: 'space-between' }}
              >
                ...
                <CardActions style={{ alignItems: 'flex-start' }}>
                  <Button
                    size='large'
                    style={{ backgroundColor: '#E5E7EB', borderRadius: '50px', textTransform: 'lowercase' }}
                  >
                    <Link
                      href={callLink(appointmentType, appointmentId)}
                      variant='body1'
                      style={{ textDecorationLine: 'none', color: '#718096' }}
                    >
                      {appointmentType === 'V' ? <VideocamRoundedIcon style={{ color: '#F08F77' }} /> : null}
                      {appointmentType === 'A' ? <PersonRoundedIcon style={{ color: '#27BEC2' }} /> : null}
                      {appointmentTransformType(appointmentType)}
                    </Link>
                  </Button>
                </CardActions>
              </Card>
            ...
            <div style={{ padding: '0.5rem' }}>
              ...
              >
                  ...
                  <CardContent>
                    <Typography style={{ flexGrow: 1 }} color='textSecondary' gutterBottom>
                      {appointmentStart.split('T')[1].split('.')[0].split(':')[0] +
                        ...
                        appointmentStart.split('T')[1].split('.')[0].split(':')[1]}
                      {calculateHours(appointmentStart.split('T')[1].split('.')[0].split(':')[0])}
                      <Link variant='body1' style={{ color: '#27BEC2', textDecorationLine: 'none' }}>
                        {' Atendido '}
                      </Link>
                ...
                <CardActions style={{ alignItems: 'flex-start' }}>
                  <Button
                    size='large'
                    style={{ backgroundColor: '#BCF0DA', borderRadius: '50px', textTransform: 'lowercase' }}
                  >
                    <Link
                      href={callLink(appointmentType, appointmentId)}
                      variant='body1'
                ...
                >
                  ...
                  <CardContent>
                    <Typography style={{ flexGrow: 1 }} color='textSecondary' gutterBottom>
                      ...
                      <DirectionsRunRoundedIcon style={{ color: '#F08F77' }} />
                    </Typography>
                    <Typography variant='body1'>
              ...
              <CardContent>
                <Typography color='textSecondary' gutterBottom style={{ flexGrow: 1, color: 'white' }}>
                  {appointmentStart.split('T')[1].split('.')[0].split(':')[0] +
                    ...
                    calculateHours(appointmentStart.split('T')[1].split('.')[0].split(':')[0]) +
                    ' - ' +
                    appointmentEnd.split('T')[1].split('.')[0].split(':')[0] +
                    ':' +
                    appointmentEnd.split('T')[1].split('.')[0].split(':')[1] +
                    calculateHours(appointmentStart.split('T')[1].split('.')[0].split(':')[0])}
      ...
      return <h2>Nuevo evento no definido</h2>
    }
    return <> </>
  }
  return <> </>
...
export function doctorData() {
  axios.get(`https://boldo-dev.pti.org.py/api/profile/doctor`).then(res => {
...
export default function LoadAppointments(today) {
  ...
  const dayUTC = today.date.getUTCDate()
  const monthUTC = '0' + (today.date.getUTCMonth() + 1)
  const yearUTC = today.date.getUTCFullYear()
  const dateCalendar = `${yearUTC}-${monthUTC}-${dayUTC}`
  const [appointments, setAppointments] = useState([])
  if (date !== today.date) {
    setDate(today.date)
  }
  console.log('recibo este dia: ' + today.date + ' y este es el dia en el estado: ' + dateCalendar)
  useEffect(() => {
    const loadPost = async () => {
      const response = await axios
        .get<{ appointments: AppointmentWithPatient[]; token: string }>(
          '/profile/doctor/appointments?start=2022-05-29T04:00:00.000Z&end=' + dateCalendar + 'T23:00:00.000Z'
        )
        .then(res => {
    ...
    <>
        ...
        <Paper variant='outlined' square style={{ borderRadius: '16px', backgroundColor: '#F9FAFB', padding: '1rem' }}>
          ...
          {appointments &&
            appointments.map(Appointment => {
              return <AppointmentType appointmentData={Appointment} date={date} />
            })}
        </Paper>
      </div>
      <div style={{ paddingTop: '20px' }}>
        <Paper style={{ backgroundColor: '#F9FAFB', padding: '1rem', borderRadius: '16px' }}>
          ...
          {appointments &&
            appointments.map(Appointment => {
              return <AppointmentTypeAfternoon appointmentData={Appointment} date={date} />
            })}
          <NoAppointments />
        </Paper>
      </div>
