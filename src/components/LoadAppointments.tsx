    ...
    const loadPost = async () => {
      const response = await axios
        .get<{ appointments: AppointmentWithPatient[]; token: string }>(
          '/profile/doctor/appointments?start=2022-05-29T04:00:00.000Z&end=2022-06-02T23:00:00.000Z'
        )
        .then(res => {
    ...
    <>
        ...
        <Paper variant='outlined' square style={{ borderRadius: '16px', backgroundColor: '#F9FAFB', padding: '1rem' }}>
          ...
          {appointments &&
            appointments.map(Appointment => {
              return <AppointmentType appointmentData={Appointment} />
            })}
        </Paper>
      </div>
      <div style={{ paddingTop: '20px' }}>
        <Paper style={{ backgroundColor: '#F9FAFB', padding: '1rem', borderRadius: '16px' }}>
          ...
          {appointments &&
            appointments.map(Appointment => {
              return <AppointmentTypeAfternoon appointmentData={Appointment} />
            })}
          <NoAppointments />
        </Paper>
      </div>
