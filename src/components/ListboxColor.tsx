...
type Props = {
  data: { value: string; name: string; color: string }[]
  label?: string
  value?: string
          ...
          <div className='relative'>
            <span className='inline-block w-full rounded-md shadow-sm'>
              <Listbox.Button className='relative w-full py-2 pl-8 pr-10 text-left transition duration-150 ease-in-out bg-white border border-gray-300 rounded-md cursor-default focus:outline-none'>
                <span className='block truncate font-semibold' title={data.find(item => item.value === selected)?.name}>
                  {data.find(item => item.value === selected)?.name}
                </span>
                <div className={`absolute inset-y-0 left-0 flex items-center pl-1.5 ml-1`}>
                  <div
                    className='h-4 w-2'
                    style={{ backgroundColor: data.find(item => item.value === selected)?.color }}
                  ></div>
                </div>
                  ...
                  <Listbox.Option key={item.value} value={item.value}>
                    {({ selected, active }) => (
                      <div className={`cursor-default select-none relative py-2 pl-8 pr-4 hover:bg-gray-100`}>
                        ...
                        <div className={`absolute inset-y-0 left-0 flex items-center pl-1.5 ml-1`}>
                          <div className='h-4 w-2' style={{ backgroundColor: item.color }}></div>
                        </div>
                      </div>
                    )}
                  </Listbox.Option>
